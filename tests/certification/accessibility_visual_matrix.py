...
_CERTIFICATION_TEST = "tests/certification/test_onboarding_accessibility_visual_certification.py"

ACCESSIBILITY_VISUAL_MATRIX = (
    AccessibilityVisualEvidence(
        "Accessible onboarding identity",
        (f"{_CERTIFICATION_TEST}::test_onboarding_surfaces_have_accessible_names",),
    ),
    AccessibilityVisualEvidence(
        "Descriptive controls",
        (f"{_CERTIFICATION_TEST}::test_primary_controls_have_names_and_tooltips",),
    ),
    AccessibilityVisualEvidence(
        "Stable object names",
        (f"{_CERTIFICATION_TEST}::test_certified_object_names_are_unique_and_stable",),
    ),
    AccessibilityVisualEvidence(
        "Consistent action language",
        (f"{_CERTIFICATION_TEST}::test_onboarding_action_language_is_consistent",),
    ),
    AccessibilityVisualEvidence(
        "Visible keyboard focus",
        (
            f"{_CERTIFICATION_TEST}::test_visible_overlays_assign_focus_to_an_action",
            "tests/certification/test_onboarding_keyboard_focus_certification.py",
        ),
    ),
    AccessibilityVisualEvidence(
        "Palette resilience",
        (f"{_CERTIFICATION_TEST}::test_onboarding_renders_with_light_and_dark_palettes",),
    ),
    AccessibilityVisualEvidence(
        "Beginner and expert consistency",
        (
            f"{_CERTIFICATION_TEST}::test_beginner_and_expert_modes_preserve_core_workspace",
    ...
    AccessibilityVisualEvidence(
        ...
        (
            f"{_CERTIFICATION_TEST}::test_validation_status_is_textual_not_colour_only",
...
def accessibility_visual_test_nodes() -> tuple[str, ...]:
    ...
    return tuple(
        dict.fromkeys(
            node for evidence in ACCESSIBILITY_VISUAL_MATRIX for node in evidence.test_nodes
        )
    )

...
def _service(
    ...
    store.save(
        AutomationProposal(
            ...
            payload={
                "required_action": "James enters the bridge.",
                "dialogue_requirement": "Report.",
                "continuity_in": "Corridor",
                "continuity_out": "At console",
    ...
    return ActionPerformanceProposalAutomationService(
        TemplateActionPerformanceProposalProvider(), store
    ), store
...
def test_performance_automation_creates_proposal_without_authority(tmp_path: Path) -> None:
    ...
    proposals = service.generate(
        story_id="STORY-001",
        source_text="James enters the bridge and says, Report.",
        source_revision="rev-1",
    )

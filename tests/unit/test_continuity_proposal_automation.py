...
def _service(tmp_path: Path) -> tuple[ContinuityProposalAutomationService, AutomationProposalService]:
    ...
    for index in (1, 2):
        ...
        store.save(AutomationProposal(
            proposal_id=f"AUT-SHOT-{index}", proposal_type=AutomationProposalType.SHOT,
            target_id=shot_id, payload={"scene_id": "EP-001-SC-001", "sequence_number": index}, provenance=provenance,
        ))
        store.save(AutomationProposal(
            proposal_id=f"AUT-PERF-{index}", proposal_type=AutomationProposalType.ACTION_PERFORMANCE,
            target_id=shot_id,
            payload={
                "opening_state": "Series entry" if index == 1 else "same as previous shot",
                "closing_state": "James at console" if index == 1 else "James turns toward Cheryl",
            }, provenance=provenance,
        ))
        store.save(AutomationProposal(
            proposal_id=f"AUT-ENV-{index}", proposal_type=AutomationProposalType.ENVIRONMENT,
            target_id=shot_id, payload={"environment_context": "interior", "surface_state": "Bridge"}, provenance=provenance,
        ))
        store.save(AutomationProposal(
            proposal_id=f"AUT-CAM-{index}", proposal_type=AutomationProposalType.CAMERA,
            target_id=shot_id, payload={"screen_direction": "left_to_right" if index == 1 else "preserve_previous"}, provenance=provenance,
        ))
        store.save(AutomationProposal(
            proposal_id=f"AUT-LIGHT-{index}", proposal_type=AutomationProposalType.LIGHTING,
            target_id=shot_id, payload={"continuity_notes": "Maintain bridge practical lighting"}, provenance=provenance,
        ))
...
def test_continuity_automation_inherits_previous_closing_state(tmp_path: Path) -> None:
    ...
    generated = service.generate(story_id="STORY-001", source_text="Bridge sequence", source_revision="rev-1")
...
def test_continuity_automation_exposes_conflict_for_human_review(tmp_path: Path) -> None:
    ...
    store.save(AutomationProposal(
        ...
        payload={"opening_state": "James is suddenly at the hatch", "closing_state": "James turns toward Cheryl"},
...
def test_continuity_automation_requires_current_camera_and_lighting(tmp_path: Path) -> None:
    service, store = _service(tmp_path)
    store._write(tuple(item for item in store.list_proposals() if item.proposal_type is not AutomationProposalType.LIGHTING))
    with pytest.raises(ValueError, match="Lighting proposals"):
        service.generate(story_id="STORY-001", source_text="Bridge sequence", source_revision="rev-1")

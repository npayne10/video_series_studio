...
def _service(
    tmp_path: Path,
) -> tuple[CameraLightingProposalAutomationService, AutomationProposalService]:
    ...
    store.save(
        AutomationProposal(
            proposal_id="AUT-ENV-1",
            proposal_type=AutomationProposalType.ENVIRONMENT,
            target_id="EP-001-SC-001-SHT-001",
            payload={
                "environment_context": "interior",
                "continuity_notes": "Controlled ship interior",
            },
            provenance=provenance,
        )
    )
    return CameraLightingProposalAutomationService(
        TemplateCameraLightingProposalProvider(), store
    ), store
...
def test_camera_lighting_requires_current_environment_proposal(tmp_path: Path) -> None:
    ...
    environment = next(
        item
        for item in store_without_environment.list_proposals()
        if item.proposal_type is AutomationProposalType.ENVIRONMENT
    )
    store_without_environment._write(
        tuple(
            item
            for item in store_without_environment.list_proposals()
            if item.proposal_id != environment.proposal_id
        )
    )
    service = CameraLightingProposalAutomationService(
        TemplateCameraLightingProposalProvider(), store_without_environment
    )

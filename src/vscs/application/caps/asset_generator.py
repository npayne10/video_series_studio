...
from vscs.infrastructure.ai.image_provider import CanonicalImageGenerationProvider
...
class CanonicalAssetGeneratorService:
    ...
    def generate(
        ...
        generated = self.provider.generate_images(asset_id, cap.title, request)
        if not generated:
        ...
        for offset, result in enumerate(generated):
            ...
            # New references intentionally enter Imported; explicitly promote generated
            # outputs to Candidate because they are ready for visual review.
            reference = self.references.mark_candidate(reference.id)
            created_ids.append(reference.id)

        self._logger.info("Generated %s canonical candidates for %s", len(created_ids), asset_id)

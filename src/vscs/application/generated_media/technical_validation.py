...
class TechnicalMediaObservation:
    ...
    def __post_init__(self) -> None:
        ...
        for name, value in (("width", self.width), ("height", self.height), ("audio_channels", self.audio_channels), ("sample_rate_hz", self.sample_rate_hz)):
...
class GeneratedMediaTechnicalRequirements:
    ...
    def __post_init__(self) -> None:
        for name, value in (("expected_width", self.expected_width), ("expected_height", self.expected_height), ("expected_audio_channels", self.expected_audio_channels), ("expected_sample_rate_hz", self.expected_sample_rate_hz)):
            if value is not None and value <= 0:
                raise ValueError(f"{name} must be positive when supplied")
        ...
        if self.minimum_duration_seconds is not None and self.maximum_duration_seconds is not None and self.minimum_duration_seconds > self.maximum_duration_seconds:
...
class GeneratedMediaTechnicalValidationService:
    ...
    def validate(
        ...
        *,
        ...
        if media.state in {GeneratedMediaState.REJECTED, GeneratedMediaState.INVALID, GeneratedMediaState.SUPERSEDED}:
        ...
        blocking = tuple(issue for issue in issues if issue.severity is TechnicalValidationSeverity.ERROR)
        if blocking:
    ...
    def _issues(
        ...
        if media.file.checksum_sha256 is not None and observation.checksum_sha256 != media.file.checksum_sha256:
            issues.append(TechnicalValidationIssue("checksum-mismatch", "Managed media checksum differs from ingestion authority."))
        if media.file.size_bytes is not None and observation.size_bytes != media.file.size_bytes:
            issues.append(TechnicalValidationIssue("size-mismatch", "Managed media byte size differs from ingestion authority."))
        if observation.size_bytes < requirements.minimum_size_bytes:
            issues.append(TechnicalValidationIssue("file-too-small", f"Media size {observation.size_bytes} is below minimum {requirements.minimum_size_bytes}."))

        suffix = "." + observation.relative_path.rsplit(".", 1)[-1].casefold() if "." in observation.relative_path else ""
        allowed_extensions = {value.casefold() if value.startswith(".") else f".{value.casefold()}" for value in requirements.allowed_extensions}
        if allowed_extensions and suffix not in allowed_extensions:
            issues.append(TechnicalValidationIssue("extension-not-allowed", f"Media extension {suffix or '<none>'} is not allowed."))
        if requirements.allowed_container_formats and (observation.container_format or "").casefold() not in {value.casefold() for value in requirements.allowed_container_formats}:
            issues.append(TechnicalValidationIssue("container-not-allowed", f"Container format {observation.container_format or '<unknown>'} is not allowed."))
        if requirements.allowed_video_codecs and (observation.video_codec or "").casefold() not in {value.casefold() for value in requirements.allowed_video_codecs}:
            issues.append(TechnicalValidationIssue("video-codec-not-allowed", f"Video codec {observation.video_codec or '<unknown>'} is not allowed."))
        if requirements.allowed_audio_codecs and (observation.audio_codec or "").casefold() not in {value.casefold() for value in requirements.allowed_audio_codecs}:
            issues.append(TechnicalValidationIssue("audio-codec-not-allowed", f"Audio codec {observation.audio_codec or '<unknown>'} is not allowed."))
        ...
        for code, label, observed, expected in (
            ...
            ("audio-channels-mismatch", "audio channels", observation.audio_channels, requirements.expected_audio_channels),
            ("sample-rate-mismatch", "sample rate", observation.sample_rate_hz, requirements.expected_sample_rate_hz),
        ):
            if expected is not None and observed != expected:
                issues.append(TechnicalValidationIssue(code, f"Observed {label} {observed!r} does not match expected {expected}."))
        if requirements.expected_frame_rate is not None and (observation.frame_rate is None or abs(observation.frame_rate - requirements.expected_frame_rate) > requirements.frame_rate_tolerance):
            issues.append(TechnicalValidationIssue("frame-rate-mismatch", f"Observed frame rate {observation.frame_rate!r} does not match expected {requirements.expected_frame_rate} within tolerance {requirements.frame_rate_tolerance}."))
        if requirements.minimum_duration_seconds is not None and (observation.duration_seconds is None or observation.duration_seconds < requirements.minimum_duration_seconds):
            issues.append(TechnicalValidationIssue("duration-too-short", f"Observed duration {observation.duration_seconds!r} is below minimum {requirements.minimum_duration_seconds}."))
        if requirements.maximum_duration_seconds is not None and (observation.duration_seconds is None or observation.duration_seconds > requirements.maximum_duration_seconds):
            issues.append(TechnicalValidationIssue("duration-too-long", f"Observed duration {observation.duration_seconds!r} exceeds maximum {requirements.maximum_duration_seconds}."))
        if requirements.require_video is True and not observation.has_video:
            issues.append(TechnicalValidationIssue("video-stream-required", "A video stream is required."))
        if requirements.require_video is False and observation.has_video:
            issues.append(TechnicalValidationIssue("video-stream-not-allowed", "A video stream is not allowed."))
        if requirements.require_audio is True and not observation.has_audio:
            issues.append(TechnicalValidationIssue("audio-stream-required", "An audio stream is required."))
        if requirements.require_audio is False and observation.has_audio:
            issues.append(TechnicalValidationIssue("audio-stream-not-allowed", "An audio stream is not allowed."))
    ...
    def _with_metadata(
        ...
        retained = tuple((key, value) for key, value in media.technical_metadata if not key.startswith(self.METADATA_PREFIX))
        ...
        values.extend((f"technical_validation.{key}", str(value)) for key, value in optional.items() if value is not None)
        for index, issue in enumerate(issues, start=1):
            stem = f"technical_validation.issue.{index:03d}"
            values.extend(((f"{stem}.code", issue.code), (f"{stem}.severity", issue.severity.value), (f"{stem}.message", issue.message)))

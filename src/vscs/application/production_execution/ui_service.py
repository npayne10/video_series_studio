...
class ProductionExecutionUiService:
    ...
    def start(self, task_id: str, production_package: Path) -> ProductionExecutionResult:
        ...
        if not normalized:
            raise ProductionExecutionError("Select a ProductionTask before starting production")
    ...
    def reconcile(self, task_id: str) -> ProductionExecutionResult:
        normalized = task_id.strip()
        if not normalized:
            raise ProductionExecutionError("Select a ProductionTask before refreshing execution")

...
from PySide6.QtWidgets import QDialog, QHBoxLayout, QMessageBox, QPlainTextEdit, QPushButton, QVBoxLayout
...
class AutomationStoryWorkspaceWidget(BrowseableStoryWorkspaceWidget):
    ...
    continuity_automation_service: ContinuityProposalAutomationService | None = None
    proposal_acceptance_service: ProposalAcceptanceService | None = None
    auto_compilation_orchestrator: ProposalAutoCompilationOrchestrator | None = None
    review_gap_detection_service: ProposalReviewGapDetectionService | None = None
    ...
    def _install_story_panel(self) -> None:
        ...
        actions = (
            ...
            ("Continuity Proposals…", "generateContinuityProposals", self._generate_continuity_proposals),
            ("Review Proposals…", "reviewAutomationProposals", self._review_proposals),
            ("Review Gaps…", "reviewAutomationGaps", self._review_gaps),
        )
        ...
        (
            ...
            self.continuity_proposals_button,
            self.review_proposals_button,
            self.review_gaps_button,
        ) = buttons
    ...
    def _set_story_actions(self, story: StoryRecord | None) -> None:
        ...
        for button in (self.planning_proposals_button, self.shot_proposals_button, self.resolve_assets_button, self.performance_proposals_button, self.environment_proposals_button, self.camera_lighting_proposals_button, self.continuity_proposals_button, self.review_proposals_button, self.review_gaps_button):
    ...
    def _generate_shot_proposals(self) -> None:
        ...
        if story is None or self.scene_shot_automation_service is None:
            self._error("Scene/Shot automation service is not registered."); return
        current = self._current_analysis(story)
    ...
    def _generate_performance_proposals(self) -> None:
        ...
        if story is None or self.action_performance_automation_service is None:
            self._error("Action/Performance automation service is not registered."); return
        current = self._current_analysis(story)
        if current is None: return
        source_text, revision, _baseline = current
        try:
    ...
    def _generate_environment_proposals(self) -> None:
        ...
        if story is None or self.environment_automation_service is None:
            self._error("Environment automation service is not registered."); return
        current = self._current_analysis(story)
        if current is None: return
        source_text, revision, _baseline = current
        try:
    ...
    def _generate_camera_lighting_proposals(self) -> None:
        ...
        if story is None or self.camera_lighting_automation_service is None:
            self._error("Camera & Lighting automation service is not registered."); return
        current = self._current_analysis(story)
        if current is None: return
        source_text, revision, _baseline = current
        try:
        ...
        QMessageBox.information(self, "Camera & Lighting Production Proposals Generated", f"Generated {cameras} Camera proposal(s) and {lighting} Lighting proposal(s).\n\nThese are reviewable proposals only. No governed Camera Plan or Lighting Plan was created, marked Ready, or approved.")

    def _generate_continuity_proposals(self) -> None:
        story = self._selected_story()
        if story is None: return
        current = self._current_analysis(story)
        if current is None: return
        source_text, revision, _baseline = current
        service = self.continuity_automation_service
        if service is None:
            service = ContinuityProposalAutomationService(AutomationProposalService(self.stories.projects))
            self.continuity_automation_service = service
        try:
            proposals = service.generate(story_id=story.story_id, source_text=source_text, source_revision=revision)
        except ValueError as exc:
            self._error(str(exc)); return
        conflicts = sum(bool(item.payload.get("continuity_conflicts")) for item in proposals)
        inherited = sum(item.payload.get("opening_resolution") in {"preserve-previous-directive", "inherited-previous-closing-state"} for item in proposals)
        QMessageBox.information(self, "Continuity-Aware Proposals Generated", f"Generated {len(proposals)} Continuity proposal(s).\nShots inheriting previous state: {inherited}.\nShots requiring human conflict review: {conflicts}.\n\nThese are deterministic reviewable proposals only. No governed Continuity compilation was created, marked Ready, compiled, or approved.")
    ...
    def _review_proposals(self) -> None:
        ...
        if not proposals:
            QMessageBox.information(self, "No Automation Proposals", "No automation proposals exist for the current Story revision."); return
        AutomationProposalReviewDialog(proposal_service, story_id=story.story_id, source_revision=revision, acceptance=self.proposal_acceptance_service, orchestrator=self.auto_compilation_orchestrator, parent=self).exec()

    def _review_gaps(self) -> None:
        story = self._selected_story()
        if story is None: return
        current = self._current_analysis(story)
        if current is None: return
        _source_text, revision, _baseline = current
        service = self.review_gap_detection_service or ProposalReviewGapDetectionService(AutomationProposalService(self.stories.projects))
        report = service.review(story_id=story.story_id, source_revision=revision)
        dialog = QDialog(self)
        dialog.setWindowTitle("AI Review, Gap Detection & Repair Suggestions")
        dialog.resize(1050, 720)
        layout = QVBoxLayout(dialog)
        text = QPlainTextEdit(dialog)
        text.setReadOnly(True)
        lines = [f"Story: {report.story_id}", f"Revision: {report.source_revision}", f"Proposals reviewed: {report.proposal_count}", f"Blockers: {report.blocker_count}", f"Warnings: {report.warning_count}", "", "Review is advisory only. No proposal or governed authority was changed.", ""]
        for gap in report.gaps:
            lines.extend([f"[{gap.severity.value.upper()}] {gap.category} — {gap.target_id}", gap.summary, f"Evidence: {gap.evidence}", f"Repair suggestion: {gap.repair_suggestion}", ""])
        if not report.gaps:
            lines.append("No deterministic proposal-chain gaps were detected.")
        text.setPlainText("\n".join(lines))

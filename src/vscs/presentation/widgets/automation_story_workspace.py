...
class AutomationStoryWorkspaceWidget(BrowseableStoryWorkspaceWidget):
    ...
    def _install_story_panel(self) -> None:
        ...
        actions = (
            ...
            (
                ...
                self._generate_camera_lighting_proposals,
            ),
            (
                "Continuity Proposals…",
                "generateContinuityProposals",
                self._generate_continuity_proposals,
            ),
    ...
    def _generate_camera_lighting_proposals(self) -> None:
        ...
        source_text, revision, _baseline = current
        try:
            proposals = self.camera_lighting_automation_service.generate(
                story_id=story.story_id, source_text=source_text, source_revision=revision
            )
        except (AIProviderError, ValueError) as exc:
            self._error(str(exc))
    ...
    def _generate_continuity_proposals(self) -> None:
        ...
        if service is None:
            service = ContinuityProposalAutomationService(
                AutomationProposalService(self.stories.projects)
            )
    ...
    def _review_proposals(self) -> None:
        ...
        AutomationProposalReviewDialog(
            ...
            story_id=story.story_id,
            source_revision=revision,
            acceptance=self.proposal_acceptance_service,
            orchestrator=self.auto_compilation_orchestrator,
            parent=self,
        ).exec()
    ...
    def _review_gaps(self) -> None:
        ...
        if current is None:
            return
        _source_text, revision, _baseline = current
        service = self.review_gap_detection_service or ProposalReviewGapDetectionService(
            AutomationProposalService(self.stories.projects)
        )
        ...
        lines = [
            ...
            f"Proposals reviewed: {report.proposal_count}",
            f"Blockers: {report.blocker_count}",
            f"Warnings: {report.warning_count}",
            "",
            "Review is advisory only. No proposal or governed authority was changed.",
            "",
        ]
        for gap in report.gaps:
            lines.extend(
                [
                    f"[{gap.severity.value.upper()}] {gap.category} — {gap.target_id}",
                    gap.summary,
                    f"Evidence: {gap.evidence}",
                    f"Repair suggestion: {gap.repair_suggestion}",
                    "",
                ]
            )
        if not report.gaps:
